...
function setDateToLocaleTime(str, end,setHour) { 
    ...
    var d = new Date();
    d.setFullYear(Number(splited[0]));
    d.setMonth(Number(splited[1]) - 1);
    d.setDate(Number(splited[2]));
    var arr = [0, 0 , 0, 0];
    if (end)
        arr = [23, 59, 59, 999];
    if (setHour)
...
function createSimblaObj(form) {
    ...
    if (T.find('.simbla-table tbody td.action .fa-check:visible').length > 0) {
        showAlert(L('Please close any open rows in order to save the form'));
        T.find('simbla-table tbody td.action .fa-check:visible').focus();
        return false;
    }
...
function simblaTable() {
    ...
    function setTdHtml(value, type, o, fixedLength) {
        ...
        switch (type) {
            case 'Date':
                return value && o.css('direction', 'ltr').text(value.toLocaleString());
    ...
    function getLimit() {
        if (m.find('.paging-area')[0].style.display === "none")
            return 100;
        ...
        m.find('table.table thead').first().find('th').each(function () {
            ...
            if (m.data('is-aggregate')) {
                ...
                if ((fieldType === "Number" && $(this).data('aggrFunc')) || $(this).data('aggrFunc') === "count") {
                    ...
                    val = d._id[$(this).data('pointer-name-key')] || d._id[key];
                }
            } else {
...
function autoRules() {
    ...
    function getFormulaVal(formula) {
        ...
        console.log("formula:", name1 + operand1, formula.operator, name2 + operand2);
    ...
    function checkConditions(rule) {
        ...
        var passTheConditions = !rule.conditions || rule.conditions.every(function (c) {
            //console.log(c);
            if (!c.field) return;
        ...
        console.log('apply rule', rule.name, 'status', passTheConditions, rule);
        try {
...
var _selectObjectsCache = {};
$(document).on('change', 'select.select-pointer', function () {
    var T = $(this);
    if (!T.val() || T.prop('multiple') || !T.attr('targetclass')) return;
    var currentObject = T.data('value-object');
    if (currentObject && currentObject.id === T.val()) return;
    // check in cache
    if (_selectObjectsCache[T.attr('targetclass')] && _selectObjectsCache[T.attr('targetclass')][T.val()]) { 
        obj = _selectObjectsCache[T.attr('targetclass')][T.val()];
        T.data('value-object', obj);
    ...
    Simbla.Object.extend(T.attr('targetclass')).createWithoutData(T.val()).fetch().then(function (obj) {
        ...
        _selectObjectsCache[T.attr('targetclass')][obj.id] = obj;
    }).catch(console.error);
